...
type:: select
options:: [P0, P1, P2, P3]
description:: Prioritization tier. P0 = do now (quick win/unblocker); P1 = next (high ROI); P2 = planned (needs explore or depends on another item); P3 = later (large or deferred).
...
The Backlog Template models a prioritized backlog document made of individually trackable work items. Each work item carries the metadata a triage workflow needs — category, size, priority tier, motivation, approach, optional requested behaviour, risks and a suggested trigger — while a single relation matrix captures how items depend on and relate to each other.
...
- Carry an explicit prioritization tier per work item so triage is data-driven, not implied by ordering.
...
| WorkItem | `priority` | select | Prioritization tier (P0/P1/P2/P3) |
...
| Tier | Meaning | Action policy |
|---|---|---|
| **P0** | Do now — quick win or unblocker | Low effort, high value, or unblocks other work / the merge path |
| **P1** | Next — high ROI | Strong value, low-to-moderate effort, not blocked |
| **P2** | Planned — needs design first | Valuable but the approach is open (`/sdd-explore`) or it depends on another item |
| **P3** | Later — large or deferred | Strategic/large work, deliberately postponed |
...
- Use `priority` for the triage tier; keep it consistent with the `depends_on` edges so sequencing falls out of the data.
- Record dependency and affinity only through the `work-item relations` matrix, never as free text.

### Prompts

- "Add a `WorkItem` for `<key>` with type `<type>`, size `<size>` and priority `<priority>`."
- "Which `WorkItem` elements are P0?"
