...
- [ ] 4. Citation provenance typing <!-- id: 4 -->
  ...
  - [ ] 4.8 Editor already supplies `resolveTemplateSchema` (`modelStore.ts`), so `validateWorkspaceSources` reads the composed schema off the root node with no new plumbing. STILL TO DO: confirm the MCP path supplies it, and give the simulacro harness a schema resolver — S07's last failure is the harness parsing without one, not a product defect <!-- id: 4.8 -->
...
- [ ] 10. Final verification <!-- id: 10 -->
  - [ ] 10.1 `npm --prefix iNNfo run typecheck` <!-- id: 10.1 -->
  - [ ] 10.2 `npm --prefix iNNfo run test` <!-- id: 10.2 -->
  - [ ] 10.3 `npm run check:integrity` and `npm run check:samples` <!-- id: 10.3 -->
  - [ ] 10.4 `node simulacro/run-all.mjs` — every scenario green except any explicitly recorded as a non-goal <!-- id: 10.4 -->
  - [ ] 10.5 Write `verify-report.md` recording what passed, what was descoped and why <!-- id: 10.5 -->
