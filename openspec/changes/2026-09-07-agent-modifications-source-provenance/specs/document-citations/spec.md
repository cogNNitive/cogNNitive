# Spec: Document Citations (Cross-Model Provenance)

## Purpose

Unify the provenance mechanism so that any knowledge block in the workspace — whether a Source under `sources/nn/` or a Model under `models/` — is addressable by the same identifier form `<workspace-relative-path>.md#<heading-slug>` and citeable by artifacts. This closes the fully traceable chain artifact → model → source, and pins the heading-level convention so model-heading slugs are meaningful and stable.
