...
- [x] 1.1 RED: Add tests in `actioNN/skills/nn-trannsform/test/unit/test-scanner.js`:
      ...
      `sources/archive/`.
- [x] 1.2 GREEN: Add `archiveSourceSnapshot(absPath, nnPath, destPath, basename)`
      ...
      `V<N>` = max existing + 1).
- [x] 1.3 GREEN: Call the snapshot from `processOkFile` and `processPromptFile`
      **before** the overwrite branch when `existingHash && existingHash !== newHash`.
- [x] 1.4 GREEN: Exclude `archive` by name in `walkOriginal` and `detectFormats`
      (same rule as `staging`).
- [x] 1.5 GREEN: In `actioNN/skills/nn-trannsform/scripts/scanner.js`, report the
      snapshot action in the registry entry (`"Archived V<N> then converted"`).
- [x] 1.6 GREEN: Orphan detection after normalization in `scanner.js`: collect
...
- [x] 1.7 Add RED tests for orphan consent in `test-scanner.js`
...
- [x] 2.1 RED: Add tests in `actioNN/skills/nn-trannsform/test/unit/test-lineage-sync.js`:
...
- [x] 2.4 GREEN: Extend `renderSourcesSection` to emit version metadata on active
      elements (`version::`, `archive_path::` from the archive scan).
- [x] 2.5 GREEN: Align `TEMPLATE_URL`/`INNFO_URL` constants to
      `cogNNitive_V_0-2-0_NN.md` and `iNNfo_V_0-2-1_NN.md`; set new record
      `model_version` to `V_0-2-0` in `buildFreshModel`.
- [x] 2.6 GREEN: Add archive-chain checks to
...
- [x] 2.7 Update `test/unit/test-provenance.js` expectations if the record
...
- [x] 3.1 Update `actioNN/skills/nn-trannsform/SKILL.md`: `sources/archive/`
...
- [x] 3.2 Update `docs/innfo/documentation/citations-provenance.md`: document
      `sources/archive/` and version metadata on `# NN Sources`.
- [x] 3.3 Update `actioNN/skills/nn-trannsform/README.md` and `TESTING.md` if
...
- [x] 4.1 Run unit suites: `node actioNN/skills/nn-trannsform/test/run.js`
      (and each touched test file individually).
- [x] 4.2 Run integration: `powershell actioNN/skills/nn-trannsform/test/test.ps1`.
- [x] 4.3 Verify write-once template untouched: confirm
      `iNNfo/specs/templates/cogNNitive/cogNNitive_V_0-2-0_NN.md` is not in the
      diff.
- [x] 4.4 Run full workspace verification: `node scripts/verify.js`.