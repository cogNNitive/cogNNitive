...
- [x] 1.4 Commit — `feat(innfo-mcp): opt-in resolver freshness check (D2)`
...
- [x] 2.6 Commit — `feat(innfo-mcp): TEMPLATE_CACHE_STALE warning on validateModel (D3)`
...
- **Archive reconciliation (sdd-archive, 2026-09-06)**: tasks 1.4, 2.6, 3.4, 4.4 (commit tasks) were checked at archive time. They were unchecked in the pre-archive state because the change landed via PR #50 (`fa74d3e`) outside the sdd-apply flow; `verify-report.md` documents they were fulfilled by the PR landing on `main`. Per the SDD archive policy (exceptional stale-checkbox reconciliation backed by verify-report), they are marked complete so the archived audit trail has no stale unchecked tasks.