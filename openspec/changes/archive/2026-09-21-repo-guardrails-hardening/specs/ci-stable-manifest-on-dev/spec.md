# Delta Spec: ci-stable-manifest-on-dev (F2)

This delta spec adds one CI check to `dev` pushes. It does not touch
`scripts/verify.js`'s `--release` gate, does not change the local
`npm run verify` contract, and does not attempt pre-merge prevention.
...
- **AND** this failure SHALL be the first automated signal of the drift
  (no earlier PR-time check exists in this change's scope).
...
- Whether the new `dev` step is blocking (fails the `dev` job) or
  report-only (annotates without failing) is an open decision the proposal
  explicitly defers to `sdd-design`. Both are compatible with the
  requirements above; the requirements describe the check's existence,
  timing, and behavior, not its exit-code handling.
