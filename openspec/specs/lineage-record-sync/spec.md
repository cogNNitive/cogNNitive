# Lineage Record Filesystem Sync

## Purpose

Make the workspace lineage record a faithful, idempotent projection of the filesystem. On every pipeline run `buildProvenanceModel` regenerates `# NN Sources`, `# NN Models`, and `# NN Artifacts` from the current state of `sources/nn/`, `models/`, and `artifacts/` (replace semantics, no duplicates), while `# NN Procedures` accumulates one append-only entry per operation. A `--check` mode reports lineage/filesystem drift and exits non-zero on any `error`-level discrepancy.
