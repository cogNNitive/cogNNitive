...
- WHEN `reconcileManifest()` runs
- THEN a new `## NN Models: Beta Business` entry carrying `<!-- nn:auto -->` is appended after the last existing entry
- AND no existing entry is reordered

#### Scenario: Deleted model file archives its entry instead of deleting it
- GIVEN an owned, marked `## NN Models: Acme Business` entry whose `path` no longer resolves to an existing file
- WHEN `reconcileManifest()` runs
...
- GIVEN a `## NN Models: Manual Entry` without `<!-- nn:auto -->`, whose `path` no longer resolves to a file
- WHEN `reconcileManifest()` runs
...
Reconciliation MUST match a discovered file to an existing `Models` entry using `normalizePathKey` applied to both the discovered file's path and the entry's `path` field.
