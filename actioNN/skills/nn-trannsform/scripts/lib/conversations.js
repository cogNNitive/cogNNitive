...
function formatSessionTimestamp(timestamp) {
  if (typeof timestamp === 'string' && /^\d{4}-\d{2}-\d{2}_\d{6}$/.test(timestamp)) {
    return timestamp;
  }
...
function reserveConversationSession(workspaceRoot, timestamp) {
  ...
  const startedAt = (timestamp instanceof Date ? timestamp : (timestamp ? new Date(timestamp) : new Date())).toISOString();
 ...
 */
function evaluateSessionDiscard({ turns = 0, modelMutations = false, mutations = false, sessionFile = null } = {}) {
...
function generateTitleSuggestions(contextOrFirstPrompt) {
  ...
  if (!text) {
    return [
      { title: 'Architecture Exploration', slug: 'architecture-exploration', recommended: true },
      { title: 'Workspace Modeling', slug: 'workspace-modeling', recommended: false },
      { title: 'System Discussion', slug: 'system-discussion', recommended: false },
    ];
  }

  const stopwords = new Set([
    'the', 'a', 'an', 'and', 'or', 'to', 'for', 'in', 'on', 'with', 'by',
    'of', 'at', 'from', 'we', 'i', 'you', 'is', 'are', 'be', 'this', 'that',
    'need', 'want', 'please', 'help', 'me', 'how', 'can', 'should',
  ]);
  ...
  if (words.length === 0) {
    return [
      { title: 'Session Discussion', slug: 'session-discussion', recommended: true },
      { title: 'Domain Modeling', slug: 'domain-modeling', recommended: false },
      { title: 'Architecture Notes', slug: 'architecture-notes', recommended: false },
    ];
  }

  const w1 = words.slice(0, Math.min(3, words.length)).join('-');
  const w2 = words.slice(Math.min(2, words.length)).join('-') || `${words[0]}-details`;
  const w3 = words.length > 2 ? `${words[0]}-${words[words.length - 1]}-review` : `${words[0]}-review`;

  const slug1 = slugify(w1) || 'session-topic';
  const slug2 = slugify(w2) !== slug1 ? slugify(w2) : `${slug1}-deep-dive`;
  const slug3 = slugify(w3) !== slug1 && slugify(w3) !== slug2 ? slugify(w3) : `${slug1}-architecture`;
 ...
 */
function finalizeConversationSession({ sessionFile, titleSlug = 'session', title = undefined, endedAt = undefined }) {
  ...
  const humanTitle = title || (titleSlug ? titleSlug.replace(/[-_]/g, ' ') : 'Conversation Session');
  ...
  let cleanSlug = titleSlug.toLowerCase().replace(/[^a-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '');
...
async function promoteConversation({
  ...
  const rawSessionContent = fs.existsSync(sessionFile) ? fs.readFileSync(sessionFile, 'utf8') : '';
