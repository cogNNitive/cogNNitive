...
tool:: [[AI Agent]]
scope:: internal
Render one preview entry per `pending` item: `id`, `kind`, target (row id, `metricType`, formula, or dependency edge), `original` vs `proposed` (or `comment` for comments and deletes). Present the preview and proceed only with explicit reviewer approval per item; mark declined items `rejected` in the working copy.
...
scope:: internal
Apply exactly one `apply_change` call per approved item with `{rationale, approved_by: "user"}`: `update_field{conceptName, elementName, fieldName, value}` for row corrections, `rename_element{…, newName}` for renames, `set_marker{…, markerName}` for marker changes. Mark each applied item `applied` in the working copy.
...
type:: data
format:: markdown
Active metrics model with approved items applied, pending validation.
...
type:: data
format:: markdown
Active metrics model after the single patch bump.
