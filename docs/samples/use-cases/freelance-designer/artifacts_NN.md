---
level: 3
parent_spec:
  name: "artifacts"
  url: "https://raw.githubusercontent.com/cogNNitive/cogNNitive/main/iNNfo/specs/templates/artifacts/spec_NN.md"
model_version: "V_1-0-0"
title: "Freelance Designer Artifacts Catalog"
---

> [!NOTE]
> This is an **iNNfo document** — a plain-text Markdown file. Open it with any text editor or view and edit it with [cogNNitive](https://cognnitive.com/innfo/app/).

# NN index

* [[Artifact]]

# NN Artifact

## NN Artifact: Interactive Client Sign-Off Dashboard
format:: html
summary:: Interactive single-page web dashboard for client specification sign-off and token verification.
status:: verified
tags:: [dashboard, html, sign-off]
produced_by:: Website Specification & Client Sign-Off Pipeline
derived_from_inputs:: Client Website Specification Model
file_path:: artifacts/interactive_spec_dashboard.html
